function fordMin(edges, nodes, end){
...
//
// const edges = [
//     { from: '458ee6a9-010e-40fd-a367-c1511b52c202', to: 'df651ca3-b0d8-4baf-8164-0f1281c986c2', label: '1', id: '16a47a66-7084-42fa-82cc-cbb72e225c73' },
...
//     { from: 'df651ca3-b0d8-4baf-8164-0f1281c986c2', to: 'ab4be705-0a50-46f9-b53a-5f27fbe74967', label: '1', id: '70066c7f-7e5d-453f-9bd0-50549e73ca9a' },
...
// ];
function handleMinClick(nodes, edges){
    removeAllColorsFromNodes();
    removeAllColorsFromEdges();
    function minimisationAlgo(nodes, edges, sourceNodeId, destinationNodeId) {
        const n = nodes.length;
        const dist = {};
        const prev = {};
        const paths = {};
// Initialiser les distances à l'infini pour tous les nœuds
...
// Boucle de relaxation
        for (let i = 0; i < n - 1; i++) {
            for (const edge of edges) {
                ...
                } else if (dist[u] !== Infinity && dist[v] === dist[u] + weight) {
...
// Supprimer les doublons dans les chemins minimaux
    ...
    const result = minimisationAlgo(nodes, edges, sourceNodeId, destinationNodeId);
    if (result) {
        console.log(`Distance minimale de ${sourceNodeId} à ${destinationNodeId} : ${result.distance}`);
        const inputElement = document.getElementById('resultat');
        inputElement.value = result.distance;
        console.log(`Chemins minimaux uniques :`);
        result.chemins.forEach(chemin => {

            idsCheminMin = chemin
            setColorOfNodeById(idsCheminMin, 'green');

            console.log(`${chemin}`);
        });

    } else {
        console.log("Impossible de trouver un chemin minimal car le graphe contient un cycle de poids négatif.");
    }
}
